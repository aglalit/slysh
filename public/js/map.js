...
var coolgradient = ['rgba(0, 255, 255, 0)', 'rgba(0, 255, 255, 1)', 'rgba(0, 191, 255, 1)', 'rgba(0, 127, 255, 1)', 'rgba(0, 63, 255, 1)', 'rgba(0, 0, 255, 1)', 'rgba(0, 0, 223, 1)', 'rgba(0, 0, 191, 1)', 'rgba(0, 0, 159, 1)', 'rgba(0, 0, 127, 1)', 'rgba(63, 0, 91, 1)', 'rgba(127, 0, 63, 1)', 'rgba(191, 0, 31, 1)', 'rgba(255, 0, 0, 1)'];
var heatmapValues = {
	7: [1, 100000],
	8: [1, 40000],
	9: [2, 40000],
...
function togglePoints() {
	if (markerstoggle) {
		markerstoggle = false;document.getElementById('markersbutton').innerHTML = 'Включить маркеры';markers.forEach(function (e) {
			e.setMap(null);
		});
	} else {
		markerstoggle = true;document.getElementById('markersbutton').innerHTML = 'Выключить маркеры';showMarkers();
	}
}
...
function initMap() {
	...
	for (var i = 0, len = data.length; i < len; i++) {
		...
		google.maps.event.addListener(marker, 'dragstart', function (event) {
			init_coords_to_drag = event.latLng.toString().replace(/\(/, '').replace(/\)/, '').split(', ').map(function (e) {
				return parseFloat(e).toFixed(6);
			});
		});
		google.maps.event.addListener(marker, 'dragend', function (event) {
			alert('Изначальные координаты: ' + init_coords_to_drag + '\n' + 'Новые координаты: ' + event.latLng.toString().replace(/\(/, '').replace(/\)/, '').split(', ').map(function (e) {
				return parseFloat(e).toFixed(6);
			}));
		});
		markers.push(marker);
